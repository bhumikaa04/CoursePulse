...
const CourseCreator = () => {
  ...
  if (!user || !user.email) {
    console.error('User email is undefined. Ensure login is successful.');
  ...
  const fetchUserCourses = async () => {
    ...
    try {
      ...
      console.log('Fetched courses:', userCourses); // Debugging line
  ...
  const handleSaveCourse = async () => {
    ...
    try {
      console.log('Sending course data:', {
        ...
        contentsCount: selectedCourse.contents?.length || 0
      });
      ...
      const response = await fetch('http://localhost:3001/create-course/save', {
        ...
        body: JSON.stringify({
          course: {
            ...selectedCourse,
            // Remove the temporary frontend id if exists
            id: undefined,
      ...
      console.log('Received saved course:', result);
  
      // Update state
      setCourses(prev => {
        // If updating existing course
        if (selectedCourse._id) {
          return prev.map(c => c._id === result._id ? result : c);
        }
        // If new course
        return [...prev, result];
      });
  ...
  const handlePublishCourse = async () => {
    ...
    try {
      ...
      const response = await fetch('/create-course/publish', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
...
const getDocumentIcon = (fileType) => {
  ...
  return (
    <div className="course-creator-container">
      ...
      {playingContent && (
        playingContent.type === 'document' ? (
          (() => {
            // Open the document in a new tab
            window.open(playingContent.url, '_blank', 'noopener,noreferrer');
            // Clear the playing content
            setPlayingContent(null);
            return null; // Render nothing since the document opens in a new tab
          })()
        ) : (
          <VideoPlayer 
